...
export default function Customers() {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      await createCustomer({
        ...
        company: form.company || undefined, gstin: form.gstin || undefined,
      })
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try {
      await deleteCustomer(deleteTarget.id); setDeleteTarget(null); load()
    } catch (err) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      ...
      <div className="relative w-full sm:max-w-xs">
        ...
        <input type="text" className="input pl-9" placeholder="Search customers…" value={search} onChange={handleSearch} />
      </div>
              ...
              <table className="w-full min-w-[420px]">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="table-th hidden lg:table-cell">GSTIN</th>
                  ...
                  {items.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50/70 transition-colors">
                      <td className="table-td">
                        ...
                        <p className="text-xs text-gray-500 mt-0.5 sm:hidden truncate max-w-[160px]">{c.email}</p>
                        {c.phone && (
                          <p className="text-xs text-gray-400 mt-0.5 sm:hidden">{c.phone}</p>
                        )}
                      </td>
                      <td className="table-td text-gray-500 hidden sm:table-cell">
                        <p className="truncate max-w-[180px]">{c.email}</p>
                        {c.phone && <p className="text-xs text-gray-400 mt-0.5">{c.phone}</p>}
                      </td>
                      <td className="table-td text-gray-500 hidden md:table-cell">{c.company || '—'}</td>
                      <td className="table-td font-mono text-xs text-gray-400 hidden lg:table-cell">{c.gstin || '—'}</td>
                      {isAdmin && (
      ...
      {/* Add Customer Modal */}
      <Modal open={addOpen} onClose={() => { setAddOpen(false); setFormError(''); setForm(EMPTY) }} title="Add Customer">
        <form onSubmit={handleAdd} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input className="input" required value={form.name} onChange={f('name')} placeholder="Customer name" />
            </div>
            <div className="sm:col-span-2">
              <label className="label">Email *</label>
              <input className="input" type="email" required value={form.email} onChange={f('email')} placeholder="customer@example.com" />
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              ...
              <textarea className="input" rows={2} value={form.address} onChange={f('address')} placeholder="Full address" />
            </div>
          </div>
